...
| [`0.18.0-to-0.19.0`](0.18.0-to-0.19.0.md) | **In progress — placeholder.** Cycle theme: closing the loop with nlink-lab's 158-arc feedback. Three breaking changes (Error factory sign normalization, `ApplyOptions` `#[non_exhaustive]`, `DeclaredLinkType` enum widening), plus the RTNETLINK twin of Plan 185 (`Connection<Route>::{into,subscribe_all}_events_with_resync` — the cycle's headline) and a `serde` feature flag. See the file for the per-plan migration table; the cycle is currently 7-plan + 3 defensive (193–195). |
