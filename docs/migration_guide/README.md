...
| [`0.23.0-to-0.24.0`](0.23.0-to-0.24.0.md) | **Depth release (sockdiag/events/nftables) — small mechanical breaking surface.** Works the 2026 roadmap pass (#170). Breaks: `ConfigDiff`/`StackDiff`/`WireguardConfigDiff` became `#[non_exhaustive]` (construct via `diff()`); `InetFilter` gained `expr`, `InetSocket` gained `cc_info` (add `..Default::default()` to literals). Behaviour: `subscribe_all()` joins the rule/nexthop/nsid/MDB groups (new `NetworkEvent` variants arrive); `InetExtension::mask()` off-by-one fixed (every `with_*` extension request was shifted one over); `TcpInfo` tail fields (`bytes_sent`, `bytes_retrans`, …) now populated instead of always-0; `WireguardConfig::diff` reports absent devices in `devices_to_add` instead of erroring; XFRM dumps stop spamming the kernel log. Headline additive: `SocketOwnerMap`/`CgroupPathMap` attribution, `SocketRateTracker`, full `INET_DIAG_BC` compiler (kernel-side ss expressions), `CcInfo` (BBR/DCTCP/vegas), `RuleExpr` decoding + `RuleInfo::counter()`, `del_*_if_exists`, `RateLimiter::reconcile`, `ensure_devices` WG bootstrap, `NamespaceSpec` facade variants. |
| [`0.24.0-to-0.25.0`](0.24.0-to-0.25.0.md) | **Bug-fix release — read the silent-behaviour-change section.** Breaks nothing at compile time and several things at runtime, because the old behaviour was wrong: the `bps` rate suffixes meant bits and now mean **bytes** (8x); a bare number in a time slot meant seconds and now means **microseconds** (`tc(8)`'s unit); HTB/TBF/police bursts were written in the wrong units (off by 15.625–125x, and `police` with a rate could not install at all); `NftablesConfig::apply` no longer deletes undeclared tables (purge is opt-in — it used to destroy Docker's and firewalld's rulesets); rules install in declaration order (`NLM_F_APPEND` was never set, so first-match-wins policies were inverted). Stricter: `FilterExpr::parse` consumes its whole input, negative rates error instead of saturating to 0. |
| [`0.25.0-to-0.26.0`](0.25.0-to-0.26.0.md) | **Bug-fix release, larger than 0.25.0 — read section 1.** The TC shaping recipes did not shape: three separate HTB `defcls` bugs sent every packet through `HTB_DIRECT`, unshaped, silently, and `limit_port` installed a **match-all** filter because `cls_flower` discards L4 keys without an ethertype. Dumps: a failed dump returned `Ok` with a short list (`NLMSG_DONE` carries the result code and nothing read it), `NLM_F_DUMP_INTR` was checked in four loops out of nine, and class/filter dumps were **always empty** (they asked for ifindex 0). Units: `LinkRates::total_bps` was 8x low, `min_bytes_for_rate` gated on a packet count (~1500x late), MPTCP ports were byte-swapped. Compile breaks: `TcHandle::CLSACT` changes value, `Mptcp*Builder::dev` and the `tuntap-async` feature are removed, `LinkRates::{rx_bps,tx_bps}` become methods. And `has_module()` stops skipping your tests — which is why the list is this long. |
| [`0.26.0-to-0.27.0`](0.26.0-to-0.27.0.md) | **Declarative-configuration release — most of it needed a *second* `apply()` to show itself.** Two compile breaks, both in `QdiscBuilder`: `tbf` takes `Rate`/`Bytes` instead of a bare `u64` that meant bytes per second (`tbf(100_000_000, …)` shaped at 800 Mbit/s), and `DeclaredQdiscType::{Netem,Tbf}` became `#[non_exhaustive]` (add `..` to a destructuring match). Silent behaviour: **purge now reaches every table the config declares into**, not just main — a VRF route that left your config and survived 0.26.0 will be removed, so dry-run the first reconcile; a declared qdisc is no longer replaced on every reconcile (byte-comparing `TCA_OPTIONS` the kernel never echoes back — steady-state `changes_made` is 0 now, statistics survive); `Stack` creates WireGuard links **and brings them up** before the network layer, and `ensure_devices` re-raises a device downed by hand; a stale `ip netns` marker is `NamespaceNotFound` rather than `EINVAL` from `setns`; `NamespaceSpec::Path` gets the `/etc/netns` overlay it documented away, and `spawn_*_with_etc` works inside containers again. Deprecated: `DsmarkConfig`/`AtmConfig` (Linux 6.8 retired both qdiscs). Additive: `apply_reconcile_with_options`, `purge_tables`, `del_route_if_exists`, `Stack::{apply,diff}_in_with`, declarative netem parity with `NetemConfig`, `list_live()`, `has_options()`. |
