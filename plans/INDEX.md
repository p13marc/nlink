...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1+2+3a+3b done: scaffold + three typed-enum codec derives + `nlink::macros` substrate + `#[derive(GenlMessage)]` covering u8/u16/u32/u64/String/Vec&lt;u8&gt;/Option&lt;T&gt;. 18 runtime tests across the substrate + derive. Remaining: `#[derive(NetlinkAttrs)]` (nested-group support, defers nesting to follow-up) + `#[genl_family]` attr macro (Phase 4) + `Connection::send_typed` (Phase 5) + example + recipe (Phase 6). **Publish before nlink** when the cycle cuts |
