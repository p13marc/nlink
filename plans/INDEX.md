...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1 + 2 done: scaffold + `GenlCommand` + `GenlAttribute` + `GenlEnum` (all three typed-enum codec derives, sharing one expansion path). 14 runtime tests + 5 trybuild compile-fail cases + 1 compile-pass. Remaining: `GenlMessage` + `NetlinkAttrs` (Phase 3) + `#[genl_family]` attr macro (Phase 4) + `Connection::send_typed` (Phase 5) + example + re-export. **Publish before nlink** when the cycle cuts |
