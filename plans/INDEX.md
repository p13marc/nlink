...
| [162](162-0.16-pool-invalidate-safety-plan.md) | `PooledConnection::invalidate(self)` consume-self fix — closes a panic-on-misuse footgun where `p.invalidate(); &*p` would panic at runtime; the new shape makes it a compile error | ~1 hour | 16 (audit) | ⚪ | – | Pre-release audit (2026-05-25) bug-hunt finding. Recommended pre-cut (~1 hour). Falls back to 0.16.1 if cut is imminent — runtime panic-on-API-misuse is acceptable for one release of soak. Source-compatible for the "invalidate then drop" use case; only breaks the bug-shape. |
| [163](163-0.16-non-exhaustive-lockdown-plan.md) | `#[non_exhaustive]` on 16 new-in-0.16 pub structs + enums (RuleInfo, NftablesDiff, ReconcileOptions, DpllDeviceReply, etc.) to preserve forward-compatibility | ~1 hour | 17 (audit) | ⚪ | – | Pre-release audit (2026-05-25) finding. **Pre-cut REQUIRED** — re-applying after publish is itself a breaking change. Only user-visible impact: `ReconcileOptions` can no longer be constructed with struct-literal; switch to `Default::default().setter(...)` builder pattern. |
| [164](164-0.17-nftables-diff-perf-plan.md) | `NftablesConfig::diff` perf — hoist `list_chains()` + `list_flowtables()` out of the table loop; O(N) → O(1) for non-rule data | ~2.5 hours | 0.17 backlog | ⚪ | – | Pre-release audit (2026-05-25) finding: O(N²+N·R) kernel round-trips for N declared tables. Pure perf, not correctness — non-blocking for 0.16. Naturally fits into the 0.17 cycle. |
| [165](165-0.16-precut-polish-plan.md) | 5 minor documentation cleanups identified by the audit: CLAUDE.md "kept current" softening, CHANGELOG migration-guide cross-link, INDEX.md count wording, master plan §2 row + frontmatter, Plan 156 test-count fix | ~20 min | 18 (audit) | ⚪ | – | Pre-release audit (2026-05-25) doc-currency findings. Single polish commit pre-cut. |
| [166](166-0.17-integration-test-backfill-plan.md) | Integration-test backfill for 0.16 features (Plans 148/149/150/153/157/158/159) — ~30 test scenarios across ~900 LOC | ~9-10 hours after Plan 140 | 0.17 backlog | ⚪ | – | Pre-release audit (2026-05-25) finding. Depends on Plan 140 / Plan 142 privileged-CI gate. Library code is correct by construction (unit tests + manual `--apply` verification); this is the regression net for the deferred root-gated test scenarios. |

Total focused-work estimate: **~35 – 37 days** original cycle +
**~5 hours** of pre-cut audit follow-up commits (Plans 161-165)
+ ~12 hours of 0.17 backlog (Plans 164, 166).
