...
| [157](157-0.16-nftables-declarative-config-plan.md) | Declarative `NftablesConfig` — `diff + apply + reconcile` mirroring `NetworkConfig`, atomic-via-`Transaction` commit, canonicalized rule equivalence | ~3 days | 12 | 🟡 | – | Core types + diff + atomic apply + `apply_reconcile(opts)` (bounded retry on EBUSY/EAGAIN per Plan §4.5) + recipe (`docs/recipes/nftables-declarative-config.md`) all landed. **Canonicalization (§4.3) deferred to 0.17** — needs a refactor of the `Rule` type, which currently stores `Vec<Expr>` already-lowered; Plan §4.3's design requires the typed `Vec<Match>` layer for sortability. Sets/maps still deferred per original plan scope |
