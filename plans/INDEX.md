...
| [154](154-0.16-nlink-macros-plan.md) | New `nlink-macros` proc-macro crate: `#[derive(GenlMessage/GenlCommand/GenlAttribute/GenlEnum/NetlinkAttrs)]` + `#[genl_family]` | ~7.5 days | 9 | 🟡 | – | Phases 1 + 2 + 3a done: scaffold + three typed-enum codec derives + `nlink::macros` module with `GenlMessage`/`NetlinkAttrs` traits + `__rt` runtime helpers + crate-level re-export of all derives. nlink → nlink-macros dependency wired. Remaining: `#[derive(GenlMessage)]` + `#[derive(NetlinkAttrs)]` derives (Phase 3b) + `#[genl_family]` attr macro (Phase 4) + `Connection::send_typed` (Phase 5) + example + recipe (Phase 6). **Publish before nlink** when the cycle cuts |
