...
| [185](185-nftables-subscribe-with-resync-plan.md) | `Connection<Nftables>::into_events_with_resync(factory)` — kube-rs-shaped watcher with built-in ENOBUFS recovery | ~3.5 h | 6 | ⚪ | **Plan refined 2026-05-30** after ecosystem research grounded the design in kube-rs `watcher`. New shape consumes `self` (matches kube-rs `Api<K>` precedent) + takes an `Arc<dyn Fn() -> BoxFuture<Result<Connection<P>>>>` factory for snapshot connections. Returns `'static + Send` stream — `tokio::spawn`-able + `StreamMap`-mergeable. Sidesteps the lifetime puzzle from the first-cut attempt. Ready to implement. |
