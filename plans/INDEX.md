...
| [153](153-0.16-kernel-feature-bundle-plan.md) | XFRM IPsec offload (`XFRMA_OFFLOAD_DEV`) + Devlink rate + port-function-state + TX H/W shaping (`net_shaper` GENL) | ~5 days | 5 | 🟡 | – | §4.1 XFRM offload landed (XfrmSaBuilder::offload + XfrmOffloadFlag bitwise newtype). Devlink rate (§4.2) + net_shaper GENL (§4.3) follow up — each ~1-2d standalone work |
...
| [158](158-0.16-syscall-batching-plan.md) | `recvmmsg`/`sendmmsg` batching behind opt-in `syscall_batch` feature; 32-frame batches; per-socket reused buffers | ~2 days | 8 | 🟡 | – | recv_batch + send_batch landed behind syscall_batch feature; dump-path wiring + criterion benches deferred (benchmark infra is its own substantial work; the recv_batch/send_batch primitives are usable directly by callers in the interim) |
