...
| [169](169-diagnostics-orphan-surfaced-gaps-plan.md) | Diagnostics-layer gaps surfaced by Plan 168's forensic read of orphan-example phantom symbols. Phases 1+2: `RouteInfo::source` + `dev_name` + `#[non_exhaustive]`; `InterfaceDiag::is_up()/has_carrier()/is_operational()`; `Srv6LocalRoute::table()`. Phase 3: `Bottleneck::score` deferred (design call). | ~1 hour | 23 (audit) | 🟢 (Phases 1+2) | – | Each phantom symbol was evidence of a UX papercut; 4 of 5 real gaps closed in 0.16 (one trivial 1-line lib gap — RTA_PREFSRC was parsed but dropped). +4 unit tests; 970 lib tests (was 966). Phase 3 (`Bottleneck::score` normalized severity 0..=1) deferred to 0.17 — needs design discussion on the combining formula. |
