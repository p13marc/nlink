...
fn list_namespaces(format: OutputFormat, _opts: &OutputOptions) -> Result<()> {
    let dir = match fs::read_dir(NETNS_RUN_DIR) {
        ...
        Err(e) => {
            return Err(rip_netlink::Error::Io(e));
        }
    };
...
fn add_namespace(name: &str) -> Result<()> {
    ...
    let file = OpenOptions::new()
        ...
        .map_err(|e| {
            if e.kind() == io::ErrorKind::AlreadyExists {
                rip_netlink::Error::InvalidMessage(format!("namespace '{}' already exists", name))
            } else {
                rip_netlink::Error::Io(e)
            }
        })?;
    drop(file);
    ...
    let result = unsafe {
        ...
        let pid = libc::fork();
        if pid < 0 {
            return Err(rip_netlink::Error::Io(io::Error::last_os_error()));
        }
...
fn delete_namespace(name: &str) -> Result<()> {
    ...
    fs::remove_file(&netns_path).map_err(|e| {
        rip_netlink::Error::InvalidMessage(format!("cannot remove namespace file: {}", e))
    })?;
...
fn exec_in_namespace(name: &str, command: &[String]) -> Result<()> {
    ...
    let netns_fd = File::open(&netns_path).map_err(|e| {
        rip_netlink::Error::InvalidMessage(format!("cannot open namespace '{}': {}", name, e))
    })?;
    ...
    let status = Command::new(&command[0])
        ...
        .status()
        .map_err(|e| {
            rip_netlink::Error::InvalidMessage(format!("failed to execute '{}': {}", command[0], e))
        })?;
...
fn identify_namespace(pid: &str) -> Result<()> {
    ...
    let target_stat = fs::metadata(&net_path).map_err(|e| {
        rip_netlink::Error::InvalidMessage(format!("cannot access process {}: {}", pid, e))
    })?;
...
fn list_pids_in_namespace(name: &str) -> Result<()> {
    ...
    let ns_stat = fs::metadata(&netns_path).map_err(rip_netlink::Error::Io)?;
    use std::os::unix::fs::MetadataExt;
    let ns_dev = ns_stat.dev();
    let ns_ino = ns_stat.ino();

    // Iterate through /proc to find matching processes
    let proc_dir = fs::read_dir("/proc").map_err(rip_netlink::Error::Io)?;
...
fn monitor_namespaces() -> Result<()> {
    ...
    loop {
        ...
        if len < 0 {
            ...
            unsafe { libc::close(inotify_fd) };
            return Err(rip_netlink::Error::Io(err));
        }
...
fn set_namespace_id(name: &str, nsid: &str) -> Result<()> {
    ...
    let id: i32 = if nsid == "auto" {
        ...
        nsid.parse()
            .map_err(|_| rip_netlink::Error::InvalidMessage(format!("invalid nsid: {}", nsid)))?
    };
...
fn create_netns_dir() -> Result<()> {
    match fs::create_dir_all(NETNS_RUN_DIR) {
        ...
        Err(e) => Err(rip_netlink::Error::Io(e)),
    }
}
