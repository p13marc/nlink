...
//!
//! This module provides the main interface for querying socket information
//! via the NETLINK_SOCK_DIAG protocol.
...
use crate::socket::{
    InetSocket, SocketInfo, UnixSocket, UnixType, parse_ipv4, parse_ipv6, parse_port,
};
...
const INET_DIAG_CLASS_ID: u16 = 17;
...
/// Socket diagnostics connection.
///
/// This provides async access to the kernel's socket diagnostics interface.
pub struct SockDiag {
    fd: AsyncFd<RawFd>,
    seq: u32,
    pid: u32,
...
impl SockDiag {
    ...
    pub async fn query(&mut self, filter: &SocketFilter) -> Result<Vec<SocketInfo>> {
        match &filter.kind {
    ...
    pub async fn query_tcp(&mut self) -> Result<Vec<InetSocket>> {
    ...
    pub async fn query_udp(&mut self) -> Result<Vec<InetSocket>> {
        let filter = InetFilter {
    ...
    pub async fn query_unix_sockets(&mut self) -> Result<Vec<UnixSocket>> {
    ...
    async fn query_inet_family(
        ...
        buf.extend_from_slice(&self.pid.to_ne_bytes()); // nlmsg_pid
        ...
        loop {
            let n = self.recv(&mut recv_buf).await?;
            let data = &recv_buf[..n];
            ...
            while offset + 16 <= data.len() {
                ...
                match nlmsg_type {
                    ...
                    SOCK_DIAG_BY_FAMILY | TCPDIAG_GETSOCK => {
                        if let Some(sock) =
                            self.parse_inet_msg(&data[offset..offset + nlmsg_len], filter.protocol)
                        {
                            sockets.push(sock);
                        }
                    }
    ...
    fn parse_inet_msg(&self, data: &[u8], protocol: Protocol) -> Option<InetSocket> {
        ...
        while attr_offset + 4 <= data.len() {
            ...
            match attr_type {
                ...
                INET_DIAG_INFO => {
                    sock.tcp_info = Some(self.parse_tcp_info(attr_data));
                }
                INET_DIAG_CONG => {
    ...
    fn parse_tcp_info(&self, data: &[u8]) -> TcpInfo {
    ...
    async fn query_unix(&mut self, filter: &UnixFilter) -> Result<Vec<SocketInfo>> {
        let sockets = self.query_unix_typed(filter).await?;
        Ok(sockets.into_iter().map(SocketInfo::Unix).collect())
    }

    async fn query_unix_typed(&mut self, filter: &UnixFilter) -> Result<Vec<UnixSocket>> {
        let seq = self.next_seq();
        ...
        loop {
            ...
            while offset + 16 <= data.len() {
                ...
                match nlmsg_type {
                    ...
                    SOCK_DIAG_BY_FAMILY => {
                        if let Some(sock) = self.parse_unix_msg(&data[offset..offset + nlmsg_len]) {
                            sockets.push(sock);
                        }
                    }
                    _ => {}
    ...
    fn parse_unix_msg(&self, data: &[u8]) -> Option<UnixSocket> {
        // Skip netlink header (16 bytes)
        if data.len() < 16 + 16 {
            return None;
        }
    ...
    async fn query_netlink(&mut self, _filter: &NetlinkFilter) -> Result<Vec<SocketInfo>> {
        // Netlink socket diagnostics - simplified for now
        Ok(Vec::new())
    }

    async fn query_packet(&mut self, _filter: &PacketFilter) -> Result<Vec<SocketInfo>> {
