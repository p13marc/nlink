...
| 137 | [Netfilter expansion](137-netfilter-expansion-plan.md) | **PR A wire-format validated**, **PR B unit-test landed** (both under `[Unreleased]`); C/D/E pending | PR A slices 1+2+3 (`ConntrackBuilder` + recipe + `--apply` example) + commit `122f60b` validation fix; `--apply` round-trips on Linux 6.19. PR B: `ConntrackEvent` / `ConntrackGroup` / `subscribe` / `EventSource for Netfilter` + 6 parse-events unit tests. Both PRs share the same deferred slice: root-gated integration tests under the `lab` feature, plus a paired `examples/netfilter/conntrack_events.rs` and recipe pointer. PRs C (`ct_expect`), D (nfqueue), E (nflog) unstarted; D/E gated on demand. |
