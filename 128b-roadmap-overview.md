...
| 141 | [XFRM write-path API extension](141-xfrm-write-path-plan.md) | **PR A slice 1 shipped** under `[Unreleased]` (`74a4e48`) — XfrmSaBuilder + add/del/flush_sa, 8 wire-format tests; PR A slice 2 (update/flush_proto/get_sa) + PRs B/C drafted | 3-PR arc. PR A: SA CRUD + 8 round-trip tests. PR B: SP CRUD + tests. PR C: `xfrm-ipsec-tunnel` recipe + example `--apply` promotion (closes Plan 135 PR B at 7/7). |
