...
| 139 | [Typed standalone-action CRUD](139-typed-standalone-action-crud-plan.md) | **PR A shipped** under `[Unreleased]` (`d69e10a`) — library typed CRUD + ActionMessage parser, 8 wire-format tests; PRs B/C drafted | 3-PR arc. PR A: `Connection<Route>::add/del/get/dump_action` + `ActionMessage` (raw `options_raw` payload — per-kind decoders deferred). PR B: per-kind `parse_params` (~14 action kinds) — bulk of the bin migration work. **PR C is the legacy-deletion milestone** (deletes `tc::builders::*` + `tc::options::*`). |
