...
| Workspace-wide rollout of typed units | Medium | Plans 129/130 landed in nlink; the bins (`bins/{tc,ip,ss,nft,wifi,devlink,bridge,wg,ethtool,diag,config}`) should migrate off any remaining string/raw-u32 patterns. Audit per-bin during implementation. **In progress**: `tc::builders::{class,qdisc,filter,action}` marked `#[deprecated]` (0.14.0); `bins/tc`'s `class` subcommand migrated to `Connection::{add,del,change,replace}_class` (typed `TcHandle`, `#[allow(deprecated)]` dropped); **six slices landed**: five `parse_params` (`HtbQdiscConfig`, `NetemConfig`, `FlowerFilter`, `CakeConfig`, `TbfConfig` — 68 unit tests) plus the **bin dispatch wiring for `qdisc.rs`** (slice 6) — the four typed qdisc kinds now route through typed `Connection::*_qdisc_full` calls, falling through to the legacy `qdisc_builder::*` for the long tail (sfq, prio, fq_codel, ingress, etc.). The `CakeConfig` work also caught and fixed a bits-vs-bytes/sec units bug in `NetemConfig::parse_params`'s `rate` token (was using `get_rate` + `Rate::bytes_per_sec`, now uses `Rate::parse` directly — same fix carried into TBF). **Next slice**: bin dispatch in `bins/tc/src/commands/filter.rs` to wire the typed `FlowerFilter::parse_params` path. After that, the only legacy callers in the bin are the long-tail qdisc kinds and the action subcommand — the latter blocked on typed standalone-action CRUD on `Connection`, the former opportunistic per-kind work. Rejected alternative: adding `Connection::add_qdisc_with_params(kind, &[&str])` to mirror the class shape — that shape is an incomplete-migration fossil. `action` subcommand remains blocked on typed standalone-action CRUD on `Connection`. |
