...
last updated: 2026-04-25 (**Plan 142 closes — 0.15.0 ready to cut**; all four Phase 4 milestones met under `[Unreleased]`: typed surface end-to-end, `tc::builders::*` + `tc::options::*` deleted (-3940 LOC), zero `#[allow(deprecated)]` in `bins/tc`, 41 typed `ParseParams` impls; remaining open items — Plan 141 PR C recipe and Plan 137 integration tests un-parking — are sudo-gated and ship post-0.15.0)
---
...
| 139 | [Typed standalone-action CRUD](139-typed-standalone-action-crud-plan.md) | **all 3 PRs shipped** — **Plan 139 closes**; the legacy-deletion milestone (`56371db`) is the **0.15.0 release-cut** | 3-PR arc complete. PR A: library typed CRUD (`d69e10a`). PR B: per-kind `parse_params` for all 14 action kinds, 74 tests (`f7e4502`/`d124920`/`2764806`). PR C: bin migration (`b2370fd`/`0d095ae`) + legacy deletion (`56371db`, -3940 LOC). All Plan 142 §6 acceptance gates met. |
