...
| 138 | [bins/tc u32 filter selector grammar](138-u32-filter-selector-grammar-plan.md) | **PR A shipped** under `[Unreleased]` (commit `ae0e4ae`) — raw triples + structural tokens, bin wired, filter side 8/9; PRs B/C drafted | 3-PR arc. PR A: raw `match u32|u16|u8 VAL MASK at OFFSET` + classid/flowid/chain/skip_hw/skip_sw, 16 unit tests. PR B: named-match shortcuts (golden-hex fixtures from `tc(8)`). PR C: hash-table grammar (`divisor`, `ht`, `link`, `order`, `hashkey`). |
