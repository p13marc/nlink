...
| 141 | [XFRM write-path API extension](141-xfrm-write-path-plan.md) | **PR A complete** under `[Unreleased]` (`74a4e48`+`844a166`) — XfrmSaBuilder + add/update/del/flush/flush_proto/get_sa, 12 wire-format tests; PRs B/C drafted | 3-PR arc. PR A: SA CRUD complete (all 6 methods + 12 round-trip tests). PR B: SP CRUD + tests. PR C: `xfrm-ipsec-tunnel` recipe + example `--apply` promotion (closes Plan 135 PR B at 7/7). |
