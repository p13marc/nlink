...
| 139 | [Typed standalone-action CRUD](139-typed-standalone-action-crud-plan.md) | **PR A shipped** + **PR B sub-slices 1+2 shipped** (`d69e10a`+`f7e4502`+`d124920`) — library CRUD + **11 of ~14 action kinds typed-first** | 3-PR arc. PR A: library typed CRUD (shipped). PR B: per-kind `parse_params` — sub-slices 1+2 (gact/mirred/vlan/skbedit/connmark/csum/sample/tunnel_key/nat/simple/bpf, 11 kinds, 59 tests) shipped; sub-slice 3 covers the trickier remaining 3 kinds (police/ct/pedit). **PR C is the legacy-deletion milestone** (deletes `tc::builders::*` + `tc::options::*`). |
