...
| 139 | [Typed standalone-action CRUD](139-typed-standalone-action-crud-plan.md) | **PRs A+B shipped** (`d69e10a`/`f7e4502`/`d124920`/`2764806`) — **all 14 action kinds typed-first**; PR C drafted (legacy-deletion) | 3-PR arc. PR A: library typed CRUD (shipped). PR B: per-kind `parse_params` for all 14 action kinds (13 fully parsed + PeditAction stub per Plan §10), 74 tests. **PR C is the legacy-deletion milestone for Plan 142 Phase 4** — migrates `bins/tc/src/commands/action.rs` to typed dispatch, drops the last `#[allow(deprecated)]` on `bins/tc`, deletes `tc::builders::*` + `tc::options::*` entirely. |
