...
| 135 | [Recipes + public `nlink::lab`](135-recipes-and-lab-helpers-plan.md) | **PR A complete**; PR B not started | `nlink::lab` public behind `lab` feature: `LabNamespace`, `with_namespace`, `LabBridge`, `LabVeth`, and the `examples/lab/three_namespace.rs` walk-through. PR B: 7 recipes (bridge VLAN, bidir rate limit, WireGuard mesh, IPsec, nftables stateful firewall, cgroup classification, multi-namespace events). The cgroup-classification recipe blocks on Plan 133 PR C. |
