...
| 137 | [Netfilter expansion](137-netfilter-expansion-plan.md) | **PRs A+B both wire-format validated** (under `[Unreleased]`); C/D/E pending | PR A slices 1+2+3 + the `122f60b` timeout fix — `--apply` round-trips on Linux 6.19. PR B: typed events + `EventSource for Netfilter` + 6 parse-events unit tests + `netfilter_conntrack_events --apply` validated on Linux 6.19 (1 NEW + 1 DESTROY arrive, same kernel-assigned ID round-trips through the multicast channel). Remaining for both PRs: root-gated integration tests under the `lab` feature + a recipe pointer for the events Stream. PRs C (`ct_expect`), D (nfqueue), E (nflog) unstarted; D/E gated on demand. |
