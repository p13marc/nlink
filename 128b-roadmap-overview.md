...
| 139 | [Typed standalone-action CRUD](139-typed-standalone-action-crud-plan.md) | **PR A shipped** + **PR B sub-slice 1 shipped** (`d69e10a`+`f7e4502`) — library CRUD + 5 of ~14 action kinds typed-first | 3-PR arc. PR A: library typed CRUD (shipped). PR B: per-kind `parse_params` — sub-slice 1 (gact/mirred/vlan/skbedit/connmark) shipped, sub-slices 2+ for the remaining 9 kinds (police/nat/tunnel_key/csum/sample/ct/pedit/bpf/simple). **PR C is the legacy-deletion milestone** (deletes `tc::builders::*` + `tc::options::*`). |
