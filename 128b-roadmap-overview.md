...
| Workspace-wide rollout of typed units | Medium | Plans 129/130 landed in nlink; the bins (`bins/{tc,ip,ss,nft,wifi,devlink,bridge,wg,ethtool,diag,config}`) should migrate off any remaining string/raw-u32 patterns. Audit per-bin during implementation. **In progress**: `tc::builders::{class,qdisc,filter,action}` marked `#[deprecated]` (0.14.0); `bins/tc`'s `class` subcommand migrated to `Connection::{add,del,change,replace}_class` (typed `TcHandle`, `#[allow(deprecated)]` dropped); **seven slices landed**: five `parse_params` (`HtbQdiscConfig`, `NetemConfig`, `FlowerFilter`, `CakeConfig`, `TbfConfig` — 68 unit tests) + bin dispatch wiring for **both** `qdisc.rs` (slice 6, four typed qdisc kinds) and `filter.rs` (slice 7, flower filter kind). All four typed verbs (add/del/replace/change) on both subcommands now route through typed `Connection::*` for known kinds, falling through to the deprecated `qdisc_builder::*` / `filter_builder::*` for the long tail (sfq, prio, fq_codel, ingress on the qdisc side; u32, matchall, basic, fw, bpf, cgroup, route, flow on the filter side). The `CakeConfig` work also caught and fixed a bits-vs-bytes/sec units bug in `NetemConfig::parse_params`'s `rate` token (was using `get_rate` + `Rate::bytes_per_sec`, now uses `Rate::parse` directly — same fix carried into TBF). **Remaining**: opportunistic per-kind `parse_params` for the long-tail kinds when they prove to be in real use; `action` subcommand still blocked on typed standalone-action CRUD on `Connection`. Rejected alternative: adding `Connection::add_qdisc_with_params(kind, &[&str])` to mirror the class shape — that shape is an incomplete-migration fossil. `action` subcommand remains blocked on typed standalone-action CRUD on `Connection`. |
