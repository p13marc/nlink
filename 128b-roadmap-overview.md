...
| 138 | [bins/tc u32 filter selector grammar](138-u32-filter-selector-grammar-plan.md) | **PRs A+B shipped** under `[Unreleased]` (`ae0e4ae`, `3b5cb21`) — raw triples + named-match shortcuts; PR C drafted | PR A: raw `match u32|u16|u8 VAL MASK at OFFSET` + classid/flowid/chain/skip_hw/skip_sw. PR B: named-match shortcuts (`match ip src/dst/protocol/sport/dport`, `match tcp|udp sport|dport`). 31 unit tests for both. PR C: hash-table grammar (`divisor`, `ht`, `link`, `order`, `hashkey`). Filter side 8/9 typed-first. |
