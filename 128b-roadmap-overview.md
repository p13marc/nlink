...
| Workspace-wide rollout of typed units | Medium | Plans 129/130 landed in nlink; the bins (`bins/{tc,ip,ss,nft,wifi,devlink,bridge,wg,ethtool,diag,config}`) should migrate off any remaining string/raw-u32 patterns. Audit per-bin during implementation. **In progress**: `tc::builders::{class,qdisc,filter,action}` marked `#[deprecated]` (0.14.0); `bins/tc`'s `class` subcommand migrated to `Connection::{add,del,change,replace}_class` (typed `TcHandle`, `#[allow(deprecated)]` dropped); **eleven slices landed**: seventeen `parse_params` (`HtbQdiscConfig`, `NetemConfig`, `FlowerFilter`, `CakeConfig`, `TbfConfig`, `SfqConfig`, `PrioConfig`, `FqCodelConfig`, `MatchallFilter`, `FwFilter`, `RedConfig`, `PieConfig`, `HfscConfig`, `DrrConfig`, `QfqConfig`, `IngressConfig`, `ClsactConfig` — 117 unit tests) + bin dispatch wiring for **both** `qdisc.rs` (slices 6+8+10+11, fourteen typed qdisc kinds) and `filter.rs` (slices 7+9, three typed filter kinds). All four typed verbs (add/del/replace/change) on both subcommands route through typed `Connection::*` for known kinds. Long tail remaining: mqprio, taprio, etf, plug on the qdisc side; u32, basic, bpf, cgroup, route, flow on the filter side. Slices 10 and 11 added net-new CLI capability — the legacy CLI couldn't use red/pie/hfsc/drr/qfq at all because its dispatcher silently swallowed unknown kinds. The `CakeConfig` work also caught and fixed a bits-vs-bytes/sec units bug in `NetemConfig::parse_params`'s `rate` token. **Remaining**: opportunistic per-kind `parse_params` for the long-tail kinds when they prove to be in real use; `action` subcommand still blocked on typed standalone-action CRUD on `Connection`. Rejected alternative: adding `Connection::add_qdisc_with_params(kind, &[&str])` to mirror the class shape — that shape is an incomplete-migration fossil. `action` subcommand remains blocked on typed standalone-action CRUD on `Connection`. |
